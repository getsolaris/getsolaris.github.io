...
```
PUT _ilm/policy/post-rollover-ilm-policy
{
  "policy": {
    "phases": {
      "hot": {
...
```
PUT _template/posts.v8-rollover
{
  "index_patterns": ["posts.v8-rollover-*"],
  "settings": {
    "index.lifecycle.name": "posts.v8-rollover-ilm-policy",
    "index.lifecycle.rollover_alias": "posts.v8-rollover",
    "number_of_shards": 18,
...
- index_patterns: posts.v8-rollover-* 패턴을 따르는 인덱스에 정책 적용 
...
- 새로운 인덱스는 posts.v7-2025-N 형식으로 생성됨. 
- 색인 시 인덱스를 명시적으로 posts.v7-2025-N으로 지정하여 문서 업데이트가 항상 동일한 인덱스에서 이루어지도록 강제.
...
const indexName = `posts.v7-2025-${indexNumber}`;
```
